...
train_data <- train_1_001

#choose validation data
valid_data <- valid_1_001
...
eps <- seq(0.01,1.0,length.out = 30)
minpts <- floor(seq(1,100, length.out = 30))

acc_class <- numeric(length(10))
acc_index <- list(length(10))

dist_class <- numeric(length(10))
dist_index <- list(length(10))
...
for (i in 1:10){
  ...
  acc_matrix <- matrix(data = NA, nrow = length(eps),ncol = length(minpts))
  #sens_matrix <- matrix(data = NA, nrow = length(eps), ncol = length(minpts))
  #fpr_matrix <- matrix(data = NA,nrow = length(eps), ncol = length(minpts))
  dist_matrix <- matrix(data = NA, nrow = length(eps),ncol = length(minpts))
  ...
  for (j in 1:length(eps)){
    for( k in 1:length(minpts)){
      ...
      spec <- conf$table[2,2]/(conf$table[2,2] + conf$table[1,2])
      fpr <- conf$table[1,2]/(conf$table[1,2] + conf$table[2,2])
  ...
  acc_class[i] <- max(acc_matrix)
  acc_index[[i]] <- which(acc_matrix == max(acc_matrix), arr.ind = TRUE)
  dist_class[i] <- min(dist_matrix)
  dist_index[[i]] <- which(dist_matrix == min(dist_matrix), arr.ind = TRUE)
